...
###Data preparation#############################################################
opt_hyperpars <- readRDS(paste0("./../../2_hyperparameter_tuning/GPBoost_spatio_temporal/data_", val_year,"/hyperpars_", val_year,".rds"))
...
coords_train <- cbind(time = as.numeric(format(X$date_versioning[!i_test], "%Y")),
...
parameters$model[1] <- "GPBoost_spatio_temporal"
